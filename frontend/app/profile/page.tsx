...
import { IconBriefcase, IconClock, IconEdit, IconMapPin, IconStar, IconTrophy, IconUpload, IconX } from "@/components/freelance-hub/icons";
...
const formatHourlyRate = (value: string | number | null | undefined, hasProfile: boolean) => {
  if (value === null || value === undefined || value === "") {
    return hasProfile ? "Belum diatur" : "Belum diatur";
  }
...
export const ProfilePage = ({ freelancerId }: ProfilePageProps = {}) => {
  ...
  const displayProfile = useMemo(() => {
    ...
    const name = resolved.user_name || (freelancerId ? "Freelancer" : (user?.name || ""));
    const location = resolved.user_location || resolved.country || (freelancerId ? "" : (user?.location || ""));
    const title = resolved.headline || "";
    const bio = resolved.bio || resolved.experience || "";
    
    // Skills dan Achievements
    const skills = resolved.skills && resolved.skills.length > 0 ? resolved.skills : [];
    const achievements = resolved.achievements && resolved.achievements.length > 0 ? resolved.achievements : [];
    
    const hourlyRate = resolved.hourly_rate ?? "";
    const ratingValue = Number(resolved.avg_rating ?? 0);
    const reviewCount = Number(resolved.total_reviews ?? 0);
    ...
    return {
      ...
      cvFileName: resolved.cv_file_name || "Belum ada CV",
  ...
  if (isLoading) {
    return (
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-pulse">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 bg-white rounded-3xl p-6 shadow-sm border border-slate-200 h-fit space-y-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2">{displayProfile.skills.map((skill: string) => (<Badge key={skill} text={skill} type="default" />))}</div>
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-4 pb-2 border-b border-slate-100">Achievements</h3>
                <div className="space-y-2">
                  {displayProfile.achievements.length > 0 ? displayProfile.achievements.map((achievement: string) => (
