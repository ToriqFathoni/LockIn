...
import { IconPlus, IconSearch } from "@/components/freelance-hub/icons";
...
export default function Page() {
  ...
  if (isLoading) {
    return (
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        <div className="w-full bg-white rounded-3xl shadow-sm border border-slate-200 p-8 text-sm text-slate-500">Memuat sesi pengguna...</div>
      </div>
    );
  }
  ...
  return (
  <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in h-[calc(100vh-5rem)]">
    {errorMessage ? (
      <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-2xl text-red-700 text-sm">{errorMessage}</div>
    ) : null}
    <div className="w-full bg-white rounded-3xl shadow-sm border border-slate-200 h-full flex overflow-hidden">
      <div className={`w-full md:w-1/3 border-r-0 md:border-r border-slate-100 flex-col bg-slate-50/50 ${selectedConversationId ? 'hidden md:flex' : 'flex'}`}>
            ...
            filteredConversations.map((conversation) => {
              ...
              return (
                <button
                  ...
                  className={`w-full block text-left p-4 border-b border-slate-100 hover:bg-white cursor-pointer transition-colors border-l-4 overflow-hidden ${isActive ? "bg-white border-l-[#8cbbed]" : "border-l-transparent"}`}
                >
                  <div className="flex items-center gap-3 w-full">
                    ...
                    <div className="flex-1 min-w-0 overflow-hidden">
                      <div className="flex justify-between items-baseline mb-1">
                        <h4 className="font-bold text-slate-800 truncate flex-1 min-w-0 pr-2">{conversation.other_user_name || "User"}</h4>
                        <span className="text-xs text-slate-400 shrink-0">{formatTime(conversation.last_message_time)}</span>
                      </div>
                      <p className="text-sm text-slate-500 truncate block w-full">{conversation.last_message || "Belum ada pesan"}</p>
                    </div>
                  </div>
                </button>
              );
      ...
      <div className={`w-full md:w-2/3 flex-col bg-white ${selectedConversationId ? 'flex' : 'hidden md:flex'}`}>
        {selectedConversation ? (
          <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white shadow-sm z-10">
            <div className="flex items-center gap-3">
              <button 
                className="md:hidden p-2 -ml-2 mr-1 text-slate-500 hover:text-slate-800 transition-colors"
            ...
            >
              ...
              <div>
                <h3 className="font-bold text-slate-800 hover:text-[#8cbbed] transition-colors">{selectedConversation.other_user_name || "User"}</h3>
                  <p className="text-xs text-slate-500 truncate max-w-[120px] sm:max-w-none">Lagi chat dengan {selectedConversation.other_user_email || "-"}</p>
                </div>
              </div>
            </div>
          </div>
